...
from .preprocessing import prepare_data_for_prediction, FEATURE_STORE
...
try:
    ...
    print("✅ Modèle chargé avec succès depuis un fichier local.")
...
def predict(request: NewLoanRequest):
    """
    Reçoit les caractéristiques d'une nouvelle demande de prêt,
    prépare les données et retourne une prédiction de défaut de paiement.
    """
    # On vérifie d'abord que le modèle et le magasin de features sont bien chargés
    if model is None or FEATURE_STORE is None:
        raise HTTPException(status_code=503, detail="Le modèle ou les données ne sont pas disponibles. L'API ne peut pas traiter de requêtes.")

    # Étape A : Préparer les données pour le client demandé
    try:
        client_data_df = prepare_data_for_prediction(
            client_id=request.SK_ID_CURR,
            new_loan_data=request.dict() # On passe toutes les données de la requête
        )
    except ValueError as e:
        # Si le client_id n'est pas trouvé, on renvoie une erreur 404 (Not Found)
